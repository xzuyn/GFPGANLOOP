...
os.system(
    "python inference_gfpgan.py -i LOOPINPUT -o LOOPTEMP\\LOOPRESULT-2x_" + MODEL + " -v " + MODEL + " -s " + FIRSTRUNSCALE
)
...
os.system(
    "python inference_gfpgan.py -i LOOPTEMP\\LOOPRESULT-2x_" + MODEL + "\\restored_imgs -o LOOPTEMP\\LOOPRESULT-2x-1x_" + MODEL + " -v " + MODEL + " -s 1"
)
...
os.system(
    "python inference_gfpgan.py -i LOOPTEMP\\LOOPRESULT-2x-1x_" + MODEL + "\\restored_imgs -o LOOPTEMP\\LOOPRESULT-2x-1x-1x_" + MODEL + " -v " + MODEL + " -s 1"
)
...
for RESULTIMAGE in os.listdir("LOOPTEMP\LOOPRESULT-2x-1x-1x_" + MODEL + "\\restored_imgs"):
    current_name, current_extension = os.path.splitext(RESULTIMAGE)
    shutil.move(
        ".\LOOPTEMP\LOOPRESULT-2x-1x-1x_" + MODEL + "\\restored_imgs\\"
        + current_name
        + current_extension,
        ".\LOOPRESULT\\" + current_name + "-2x-1x-1x_" + MODEL + current_extension,
    )
